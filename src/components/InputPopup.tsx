    ...
    <section className="input-popup">
      <div className="input-popup-container">
        ...
        <div className="slot-count-container">
          <input type="number" value={slotCount} id="answer-count" onChange={(e) => setSlotCount(Math.min(Math.max(Number(e.target.value), 1), 10))}/>
          <button type="button" onClick={() => setSlotCount(prevCount => Math.max(prevCount - 1, 1))}>-</button>
          <button type="button" onClick={() => setSlotCount(prevCount => Math.min(prevCount + 1, 10))}>+</button>
        </div>
        <p>1~10</p>
        ...
        <button className="add-button" type="button" onClick={() => handleAdd()}>리스트 생성</button>
      </div>
    </section>
  );
};
